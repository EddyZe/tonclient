...
func (r *PoolRepository) FindByOwnerId(ownerId int) *[]models.Pool {
	...
	var pools []models.Pool
	tx := r.db.MustBegin()
	if err := tx.SelectContext(ctx, &pools, "select * from pool as p join usr as u on p.owner_id = u.id where u.id = $1", ownerId); err != nil {
		log.Error("Error while getting pool: ", err)
		if er := tx.Rollback(); er != nil {
			log.Error("Error while rolling back transaction: ", er)
		}
...
func (r *PoolRepository) FindByOwnerIdLimit(ownerId, offset, limit int) *[]models.Pool {
	...
	var pools []models.Pool
	tx := r.db.MustBegin()
	if err := tx.SelectContext(ctx, &pools, "select * from pool as p join usr as u on p.owner_id = u.id where u.id = $1 offset $2 limit $3", ownerId, offset, limit); err != nil {
		log.Error("Error while getting pool: ", err)
		if er := tx.Rollback(); er != nil {
			log.Error("Error while rolling back transaction: ", er)
		}
